...
public class UsbDeviceMonitorService extends Service {
    ...
    private int mDetectionModeOverride = -1;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_SET_DETECTION_MODE.equals(action)) {
            ...
            if (mode >= MODE_AUTO && mode <= MODE_OTG) {
                boolean changed = mode != getDetectionMode();
                if (permanent) {
                    mDetectionModeOverride = -1;
                    mPrefs.edit().putInt(PREF_DETECTION_MODE, mode).apply();
                } else {
                    mDetectionModeOverride = mode;
                }
                if (changed) {
                    mStateMachine.sendMessage(UsbPortStateMachine.DETECTION_MODE_CHANGED);
                }
            }
        }
    ...
    int getDetectionMode() {
        if (mDetectionModeOverride >= 0) {
            return mDetectionModeOverride;
        }
    ...
    void fireDevicesDisconnected() {
        ...
        // override only is valid for a single detection cycle
        mDetectionModeOverride = -1;
    }
